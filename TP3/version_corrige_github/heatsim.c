...
//	TODO("lab3");
	 ...
	 */
		...
		//ctx->curr = cart2d_get_grid(ctx->cart, coords[0], coords[1]);
		int rank;
		...
		int waitNum = 4 *(ctx->numprocs-1);
		MPI_Waitall(waitNum, req, status);
...
void exchng2d(ctx_t *ctx) {
	...
	MPI_Request req[8];
	MPI_Status status[8];
	double *north_send_buffer = grid->dbl + pw*pad + pad;
	double *south_send_buffer = grid->dbl + (ph - pad - 1)*pw + pad;
	double *east_send_buffer = north_send_buffer + width - 1;
	double *west_send_buffer = north_send_buffer;

	double *north_recv_buffer = north_send_buffer - pw;
	double *south_recv_buffer = south_send_buffer + pw;
	double *east_recv_buffer = east_send_buffer + 1;
	double *west_recv_buffer = west_send_buffer - 1;

	MPI_Irecv(north_recv_buffer, width, MPI_DOUBLE, ctx->north_peer,11, comm, &req[0]);
	MPI_Irecv(south_recv_buffer, width, MPI_DOUBLE, ctx->south_peer,10, comm, &req[1]);
	MPI_Irecv(east_recv_buffer, 1, ctx->vector,	ctx->east_peer,		13, comm, &req[2]);
	MPI_Irecv(west_recv_buffer, 1, ctx->vector, ctx->west_peer,		12, comm, &req[3]);


  	MPI_Isend(north_send_buffer, width, MPI_DOUBLE, ctx->north_peer,10, comm, &req[4]);
  	MPI_Isend(south_send_buffer, width, MPI_DOUBLE, ctx->south_peer,11, comm, &req[5]);
	MPI_Isend(east_send_buffer, 1, ctx->vector, ctx->east_peer, 	12, comm, &req[6]);
	MPI_Isend(west_send_buffer, 1, ctx->vector, ctx->west_peer,		13, comm, &req[7]);
