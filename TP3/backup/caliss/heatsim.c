...
void exchng2d(ctx_t *ctx) {
	...
	double *offset_send = data + width+1;
	double *offset_recv = data + (height - 1) * width+1;
    MPI_Sendrecv(offset_send, width, MPI_DOUBLE, north, 2, offset_recv, width, MPI_DOUBLE, south, 2, comm, &status[1]);
    // Exchange north->south
    offset_send = data + (height - 2) * width+1;
    offset_recv = data+1;
    MPI_Sendrecv(offset_send, width, MPI_DOUBLE, south, 1, offset_recv, width, MPI_DOUBLE, north, 1, comm, &status[0]);
    // Column exchange
    // Exchange east->west
    offset_send = data + 2*width;
    offset_recv = data + (width);
    MPI_Sendrecv(offset_send, 1, ctx->vector, west, 3, offset_recv, 1, ctx->vector, east, 3, comm, &status[2]);
    // Exchange west->east
    offset_send = data + (width - 1);
    offset_recv = data + 2*width+1;
    MPI_Sendrecv(offset_send, 1, ctx->vector, east, 4, offset_recv, 1, ctx->vector, west, 4, comm, &status[3]);
}
